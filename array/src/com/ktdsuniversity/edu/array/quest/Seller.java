	...
	public void sale() {
		...
		while (true) {
			...
			for (int i = 0; i < Goods.length; i++) { // 배열의 길이만큼 반복
				/*
				 * 입력한 상품의 이름이 같고 상품재고가 구매할 상품의 갯수보다 크거나 같고 재고가 0이 아닐 경우 구매할 갯수 만큼 재고에서 차감한 새로운
				 * 인스턴스를 만들어 상품이 위치한 배열의 순서에 새로운 인스턴스를 할당합니다
				 */
				if (Goods[i].getName().equals(saleProduct)) {// 배열에 저장된 상품의 이름과 구매할려는 상품의 이름이 같을시

					if (Goods[i].getQuantity() >= buyCount && Goods[i].getQuantity() != 0) { // 재고가 구매수량보다 많고 "그리고" 재고가
																								// 0이 아닌경우
						Goods[i] = new Product(saleProduct, Goods[i].getPrice(), Goods[i].getQuantity() - buyCount);
						break; // 상품 구매 후 for문을 빠져나옵니다

					} else if (Goods[i].getQuantity() < buyCount || Goods[i].getQuantity() == 0) { // 재고보다 구매수량이 "많거나"
																									// 재고가 0인경우
